 ...
 * The name is required, if it is not provided or property wasn't registered before,
...
/**
 * Running application. Receives base directory and configs object as parameters.
 *
 * @param {String} baseDirectory Base directory to be passed to __twee.setBaseDirectory()__ method.
 * @param {Object} options Options to be passed to __twee.Bootstrap()__ method.
 * @chainable
 * @method
 * @member twee
 * @returns {twee}
 */
twee.prototype.run = function(baseDirectory, options) {
 ...
 *
 *  * __twee__ - internal framework configuration. Usually it is combination of __twee/configs/default.json__, __application/configs/twee.(js|json)__
 *    and __application/configs/&lt;NODE_ENV&gt;/twee.(js|json)__ files. All the internal configuration is stored in following namespace: __twee:options__.
 *    You always can print all the configuration as following: __console.dir(twee.getConfig("twee:options"))__.
 *  * __module-name__ - module-specific configuration, for example: __blog_ or __forum__. It is always lower-cased.
 *  * __extension-name__ - extension-specific configuration, for example __twee-i18n__.
 *    This name depends of extension.__configNamespace__ which you should look in extension README
 *
 ...
 *
 * If some application module has it's configuration file with for example
 * name __application/modules/Blog/setup/configs/common.json__ then
 * example of getting this config file contents as JSON object is following:
 *
 *      twee.getConfig('blog:common')
 *
 * If we write new extension, then first part of config path should follow pattern: __twee-&lt;extension-name&gt;__:
 *
 *      twee.getConfig('twee-extension-name')
 *
 * If we write new module, then pattern should be: __&lt;module-lowercased-name&gt;:&lt;file-name-without-extension&gt;__:
 *
 *      twee.getConfig('module-lowercased-name:file-name-without-extension')
 *
 * @chainable
 * @method
 * @member twee
 * @param {String} key Configuration key
 * @param {*} defaultValue Bu default is __undefined__
 * @returns {*}
 */
 ...
 *      })
 *
 * In middleware (and controller action):
 *
 *      function(req, res) {
 *          res.send(res.locals.hello('TWEE.IO');
 *      }
 *
 * In controller action or controller.init():
 *
 *      this.twee.helper.hello('TWEE.IO');
 *
 * In extension:
 *
...
/**
 * Setting base directory for application. Also trying to extract __NODE_ENV__ variable and set it to
 * twee.__env field.
 *
 * If __NODE_ENV__ did not set up then __production__ value will be used by default.
 *
 * Current base directory identified with following priority:
 *
 *  * provided __directory__ param
 *  * twee.__baseDirectory - if already sat to non-empty string
 *  * process.cwd() value
 *
 * @chainable
 * @method
 ...
 *      }
 *
 * By default if your application has been installed to __./application/__ folder,
 * then configs are following:
 *
 *      {
 *          modulesConfig: 'configs/modules',
 ...
 * modules, extensions, middleware functions, initialises controllers and actions.
 *
 ...
 *
 * Sequence of configuration rewrite flow is following:
 *
 *  1. twee/configs/default.json
 *  2. ./application/configs/twee
 *  2. ./application/configs/&lt;NODE_ENV&gt;/twee
 *
 * @chainable
 * @method
 * @member twee
...
/**
 * Loading modules information using modules configuration which has been read from
 * __application/configs/modules.(json|js)__
 *
 * @chainable
 * @method
 * @member twee
 ...
 *
 * If maximum iteration deepness has been reached then __Error__ throws.
 *
 * What actually this method do - it converts extensions dependencies tree to plain array of their indexes.
 *
 * For example if we have following extensions sat up:
 *
 *      "extensions": [
 *          {
 *              "name": "ext1",
 ...
 *      ]
 *
 * Where indexes of extensions are:
 *
 *      ext1 => 0
 *      ext2 => 1
 *      ext3 => 2
 *
 * Then after resolving "isDependencyFor" the array will be following:
 *
 *      "extensions": [
 *          {
 *              "name": "ext1",
 ...
 *      ]
 *
 * And after resolving sequence of extensions loading all of them will be loaded with following indexes sequence:
 *
 *      ext2 => 1
 *      ext3 => 2
 *      ext1 => 0
 *
 * Because first will be touched ext1, it depends of ext3, and ext3 depends of ext2.
 *
 * Indexes sequence is build by calling twee.__addExtensionSequence function.
 *
 * __"isDependencyFor"__ is used for cases when you need your specific extension to be run __before__ other extension(s).
 *
 * __"dependencies"__ is used when you need your extension to be run __after__ other dependency extension(s).
 *
 * @chainable
 * @method
 * @member twee
 * @param {Integer} startFrom From what extension we should start next dependency resolving
 * @private
 */
...
/**
 * Adding extension to array of loading sequence. Also extension name is stored for debugging purposes.
 *
 * After resolving all the extensions dependencies this sequence will be used to load all the extensions in proper sequence.
 *
 * @chainable
 * @method
 * @member twee
 ...
 * returns rendered 404 template. Otherwise if no view engine us used - then plain text message will be returned.
 *
 * #### EVENTS
 *
 ...
 *
 * Extension module/file should export function which receives only one param: twee.
 * Then TWEE.IO will be able to initialise extension with instance of kernel object.
 *
 * #### EVENTS
 *
...
twee.prototype.__setupParams = function(params, router, moduleName) {
    ...
    if (params && params instanceof Object) {
        for (var param in params) {
            ...
            if (!isInternalParam && typeof paramValue === 'object') {
                // This is file module
 ...
 *
 * Example of Config __application/modules/Default/setup/setup.js__:
 *
 *      {
 *           // Module Global Middleware that are installed before any controller actions
 *           "middleware": {
 *              "head": [
 *                  {
 *                     "name": "i18nMiddleware",
 ...
 *
 *                      // Only POST and PUT http methods
 *                      "IndexController.postAction.post,put",
 *                  ],
 *
 *                  "middleware": {
 *                      "before": [
 ...
 *      }
 *
 * Routes-specific middleware functions are two types: __before__ and __after__.
 *
 * __before__ middleware are used in the same chain with controller action, but before the action.
 *
 * __after__ middleware are used in the same chain with controller action, but after the action.
 *
 * So final chain will look like:
 *
 *      app.all(pattern, [beforeMiddleware1, beforeMiddleware2, controllerAction, afterMiddleware1, afterMiddleware2]);
 *
 * Global module middleware functions are installed according two types: __head__ and __tail__.
 *
 * For example we have two application modules: __Blog__ and __Forum__. And __Blog__ module has __prefix=/blog/__, when
 * __Forum__ module has __prefix=/forum/__.
 *
 * The sequence of installing middleware functions will be following:
 *
 *  * blog.head - for __/blog/__ url
 *  * forum.head - for __/forum/__ url
 *  * blog.routes.before, blog.routes.controller.action1,..., blog.routes.after - for __/blog/&lt;pattern&gt;/__ url
 *  * forum.routes.before, forum.routes.controller.action1,..., forum.routes.after - for __/forum/&lt;pattern&gt;/__ url
 *  * blog.tail - for __/blog/__ url
 *  * forum.tail - for __/forum/__ url
 *
 * @chainable
 * @method
 * @member twee
 * @param {String} moduleName Module Name
 * @param {String|RegExp} prefix Module request prefix
 * @returns {twee}
 */
 ...
 * @returns {{controller_name: String, action_name: String, methods: Array, registryControllerName: String, moduleName: String}}
 * @private
 */
...
/**
 * Getting middleware array that should be placed __before__ or __after__ some specific middleware or controller-action middleware.
 *
 * For more detailed description of middleware types look at __twee.__setupRoutes()__ method.
 *
 * Here is structure of typical middleware settings object:
 *
 *      "middleware": {
 *          "before": [
 *              {
 *                  // The name is required, but if it isn't presents here then
 *                  // middleware.module or middleware.file will be used instead
 *                  "name": "MyMiddleware",
 *
 *                  // If your middleware is simple file (first priority).
 *                  // It is application specified middleware (not in node_modules)
 *                  // And it is usually placed in application/modules/<appModule>/middleware/myMiddleware.js
 *                  "file": "myMiddleware",
 *
 *                  // OR middleware can be located in module (second priority if both .file and .module exists)
 *                  "module": "express/some/middleware",
 *
 *                  // As additional you can pass field name of object that should contain needed middleware
 *                  // It can be nested like: myobject.mysubobject.mymethod
 *                  "method": "myMethod",
 *
 *                  // If this value has been set to true and our method is nested
 *                  // and placed in myobject.mysubobject.mymethod, then this will happend:
 *                  // mysubobject = new mysubobject;
 *                  // mymethod = mymethod.bind(mysubobject)
 *                  "reference": true,
 *
 *                  // You can disable middleware by passing this value as true
 *                  "disabled": true,
 *
 *                  // This param means that final extracted function should be called.
 *                  // For example if extracted function as myMiddleware, then
 *                  // Framework should also do: myMiddleware = myMiddleware()
 *                  // (if no middleware.params specified)
 *                  "construct": true,
 *
 *                  // If this section has been specified then middleware should be called as function to return
 *                  // Express middleware initialised as we need.
 *                  // So finally if middleware.construct has been set to true, following params section will do this:
 *                  // myMiddleware = myMiddleware.apply(null, [{someKey: "someValue", SomeKey1: twee.getConfig('twee:options:http')}]
 *                  "params": {
 *                      "someKey": "someValue",
 *
 *                      "SomeKey1": "@twee:options:http"
 *                  }
 *
 *                  // As you understand @twee.options:http with @ character in the beginning
 *                  // means to replace this string with
 *                  // real configuration value. And this mechanism supports any deepness.
 *              }
 *          ]
 *      }
 *
 * All the middleware functions that has been extracted are stored in cache, so this process will never repeats.
 *
 * @chainable
 * @method
 * @member twee
...
twee.prototype.__getMiddlewareUniqueId = function(middleware) {
    ...
    if (middleware.construct) {
        middlewareIdObject.c = 1;
    }

    if (middleware.params) {
        middlewareIdObject.p = JSON.stringify(middleware.params);
    }
...
/**
 * Makes all the magic and returns final middleware function to be used as middleware for Express core.
 *
 * All the details about extracting final middleware function look in twee.__getMiddlewareInstanceArray()
 *
 * @chainable
 * @method
 * @member twee
...
twee.prototype.__resolveMiddlewareMethod = function(middleware, moduleName) {
    ...
    if (methodParts.length) {
        ...
        for (var index = 0; index < methodParts.length; index++) {
            ...
            if (index < methodParts.length - 1 && method instanceof Object) {
                method = new method;
            }
        }
